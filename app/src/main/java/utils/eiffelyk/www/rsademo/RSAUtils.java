...
public final class RSAUtils {
    ...
    public static byte[] encryptData(byte[] data, PublicKey publicKey) {
        try {
            Cipher cipher = Cipher.getInstance("RSA/None/PKCS1Padding");//同一个明文每次生成的加密串不一致
            //Cipher cipher = Cipher.getInstance("RSA/None/NoPadding");//同一个明文每次生成的加密串一致
            // 编码前设定编码方式及密钥
